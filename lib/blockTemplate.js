...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients, auxMerkleTree) {
    ...
    // claim trie used by lbc
    var claimTrieReversed;
    if (rpcData.claimtrie) {
        claimTrieReversed = util.reverseByteOrder(new Buffer(rpcData.claimtrie, 'hex')).toString('hex');
    }
    ...
    this.serializeHeader = function (merkleRoot, nTime, nonce) {
        // lbc header has extra 32 bytes
        if (claimTrieReversed) {
            ...
            header.write(claimTrieReversed, position += 32, 32, 'hex');
    ...
    this.getJobParams = function () {
        if (!this.jobParams) {
            // lbc has additional field for claim trie
            if (claimTrieReversed) {
                this.jobParams = [
                    this.jobId,
